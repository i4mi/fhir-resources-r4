...
`getFullName(name, exludeTitles?, shortenMiddleNames?): string` extracts the full name from a HumanName, with all given names separated by a whitespace and in the end followed by the family name. Example: Homer Jay Simpson. Set the optional excludeTitles parameter to true to omit  titles in prefix and suffix. Set the optional shortenMiddleNames parameter to true to shorten the non-first given names (example: Homer J. Simpson).
...
`validate(resource, options)` validates a resource with [validator.fhir.org](https://validator.fhir.org) API. This asynchonius call may take some seconds and returns a promise of the validation outcome. This includes a short string with the validation summary, the level of validation (FATAL, ERROR, WARNING or OK) and an array of validation issues as generated by the API.
