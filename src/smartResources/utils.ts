import { Extension, Element, Coding, CodeableConcept, code, HumanName, HumanNameNameUse, Period, Patient, Identifier, Resource } from "../definition";
...
*/
export function getFullName(name: HumanName | undefined, excludeTitles = false, shortenMiddleNames = false): string {
    if (!name) return '';

    let text = '';
    if (name.prefix && name.prefix.length > 0 && !excludeTitles) text += name.prefix.reduce((a,b) => a + ' ' + b) + ' ';
    if (name.given) text += name.given.reduce((a,b) => a + ' ' + (shortenMiddleNames ? (b.substring(0,1) + '.') : b)) + ' ';
...
export function isInPeriod(period: Period, time: string | number | Date = new Date()): boolean {
  ...
  /**
   * Makes a call to the validator.fhir.org validator to check your resource.
   * Cave: The validator may take some seconds to respond.
   * @param resource    the resource you want to validate
   * @param options     validation options:
   * @returns           A promise with the validation results:
   *                    - summary: A short text summary
   *                    - level: the level of the outcome: FATAL, ERROR, WARNING or OK
   *                    - issues: an array of issues generated by the validator
   */
  export function validateResource(resource: Resource, options: ValidatorOptions): Promise<ValidatorResult> {
  const profiles = [...(resource.meta?.profile || []), ...(options.implementationGuides || [])];
  const url = options.validatorUrl || 'https://validator.fhir.org/validate';
  return new Promise((resolve, reject) => {
    const data = {
      cliContext: {
        sv: options.fhirVersion ? options.fhirVersion : '4.0.1',
        ig: profiles.length > 0 ? profiles : undefined,
        locale: options.locale
      },
      filesToValidate: [
        {
          fileName: 'toValidate.json',
    ...
    xhr.addEventListener('readystatechange', function () {
      if (this.readyState === 4) {
        if (this.status >= 200 && this.status < 300) {
          ...
          const outcome = JSON.parse(this.response).outcomes[0];
          outcome.issues.forEach((issue: {level: string}) => {
            switch (issue.level) {
              case 'FATAL':
                return resolve({
                  summary: 'Validation failed with a fatal issue.',
                  level: ValidatorResultLevel.FATAL,
          ...
          let level =
            error > 0
              ? ValidatorResultLevel.ERROR
              : warning > 0
              ? ValidatorResultLevel.WARNING
              : ValidatorResultLevel.OK;

          let summaryText = '';
          switch (level) {
            case ValidatorResultLevel.ERROR:
              summaryText = 'Validation failed with ' + error + ' errors and ' + warning + ' warnings.';
              break;
            case ValidatorResultLevel.WARNING:
              summaryText = 'Validation passed with ' + warning + ' warnings.';
              break;
            case ValidatorResultLevel.OK:
              summaryText = 'Validation passed without errors or warnings.';
              break;
          }
          ...
          console.error('Validation request failed', this.response);
          return reject('Validation request failed with status' + this.status);
        }
      }
    });
  ...
  export enum ValidatorResultLevel {
    'FATAL' = 'fatal',
    'ERROR' = 'error', 
    'WARNING' = 'warning',
    'OK' = 'ok'
  ...
  export interface ValidatorResult {
    summary: string;
    level: ValidatorResultLevel;
    issues: any[];
  };

  /**
   * Options for validateResource
   * @param fhirVersion             the FHIR version to validate against
   *                                (default: "4.0.1")
   * @param implementationGuides    the implementation guides to validate against
   *                                (default: the profiles in resource.meta.profile)
   * @param locale                  the locale (default: none)
   * @param validatorUrl            url of the validator server (default "https://validator.fhir.org/validate")
   */
